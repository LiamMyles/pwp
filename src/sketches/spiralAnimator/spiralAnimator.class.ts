...
interface SpiralAnimatorConstructor {
  spiral: NGonSpirals
}
...
export class SpiralAnimator extends SpiralDrawer {
  ...
  constructor({ spiral }: SpiralAnimatorConstructor) {
    super({ spiral })
  }
  reset(): void {
  ...
  drawActiveLines(p5: typeP5): void {
    this.spiral.verticesMatrix
      .slice(this.currentLineDrawn, this.currentLineDrawn + this.linesPerDraw)
      .forEach((_, count) => {
        const { x: subX, y: subY } =
          this.spiral.verticesMatrix[this.currentLineDrawn + count] ??
          this.spiral.verticesMatrix[this.spiral.verticesMatrix.length - 1]

        const nextPoint =
          this.spiral.verticesMatrix[this.currentLineDrawn + count + 1]
        if (nextPoint) {
  ...
  incrementDrawing(): void {
    ...
    this.totalAnimationTicks =
      this.spiral.verticesMatrix.length / this.linesPerDraw
    const tickLength =
